...
export function SchoolCalibrationsTab() {
  ...
  const { data: calibrations = [], isLoading } = useQuery<SchoolCalibration[]>({
    ...
    queryFn: () => apiClient.get(adminRoutes.calibrations()),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(adminRoutes.calibrationById(id)),
    onSuccess: () => {
  ...
  const bulkMutation = useMutation({
    mutationFn: async (items: Array<{ schoolId: string; multiplier: number; reason?: string }>) => {
      const result = await apiClient.post(adminRoutes.calibrationsBulk(), { items });
