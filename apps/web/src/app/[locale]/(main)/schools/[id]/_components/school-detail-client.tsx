...
import { SchoolHeroHeader } from './school-hero-header';
import { SchoolOverviewTab } from './school-overview-tab';
import { SchoolAcademicsTab, SchoolEssaysTab } from './school-academics-tab';
import { SchoolCasesTab } from './school-cases-tab';
...
/**
 * The interactive half of the school detail page. page.tsx is a server
 * component now and prefetches the school, so this renders real content on its
 * very first pass — which is what puts it in the server-rendered HTML.
 * `initialSchool` is null when that prefetch failed; the useQuery below then
 * runs client-side exactly as it did before.
 */
export function SchoolDetailClient({ initialSchool }: { initialSchool: SchoolDetail | null }) {
  ...
  const {
    ...
    enabled: !!schoolId,
    initialData: initialSchool ?? undefined,
  });
