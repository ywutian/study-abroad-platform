 ...
 *      - Elite (< 10%): 0.70× need-blind / 0.45× need-aware — the
 *        original peer-school calibration applies (HYPSM, NYU, top T15);
 *        calibrated to CDS data: Cornell 0.41×, Northwestern 0.68×,
 *        Columbia 0.64× → conservative midpoint 0.45× for need-aware
...
export function intlMultiplier(
  ...
  // Calibrated from CDS 2024-25: Cornell 0.41×, Northwestern 0.68×, Columbia 0.64×
  // → conservative midpoint 0.45× for need-aware elite schools.
  if (school.needBlindInternational) {
    return {
      multiplier: 0.7,
      label: 'International (need-blind, elite school)',
      evidence:
        'International pool sees ~0.7× the domestic admit rate at need-blind-for-intl elite schools (Harvard, MIT, Princeton, Yale, Amherst, Dartmouth)',
      impact: 'negative',
    };
  }
  return {
