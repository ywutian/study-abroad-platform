...
describe('HallRankingService', () => {
  ...
  describe('getPublicProfiles', () => {
    it('should return anonymized userId for ANONYMOUS profiles', async () => {
      ...
      expect(result.data[0].userId).toMatch(/^anon-/);
      expect(result.data[0].userId).not.toBe('user-1');
    });

    it('should filter by search term on targetMajor', async () => {
      mockPrisma.profile.findMany.mockResolvedValue([]);
    ...
    it('should respect pagination limits', async () => {
      ...
      const result = await service.getPublicProfiles(undefined, 2, 5);
    ...
    it('should clamp pageSize to max 100', async () => {
      ...
      const result = await service.getPublicProfiles(undefined, 1, 999);
