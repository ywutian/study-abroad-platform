...
import { AiService, ProfileAnalysisRequest, EssayReviewRequest } from './ai.service';
...
export class AiController {
  ...
  async analyzeProfile(@CurrentUser() user: CurrentUserPayload, @Body() data: ProfileAnalysisDto) {
    return this.aiService.analyzeProfile(data);
  }

  @Post('review-essay')
  @ApiOperation({ summary: '文书评估' })
  async reviewEssay(@CurrentUser() user: CurrentUserPayload, @Body() data: EssayReviewDto) {
    return this.aiService.reviewEssay(data);
  }

  @Post('generate-ideas')
  @ApiOperation({ summary: '生成文书创意' })
  async generateIdeas(@CurrentUser() user: CurrentUserPayload, @Body() data: GenerateIdeasDto) {
    const ideas = await this.aiService.generateEssayIdeas(data.topic, data.background);
    return { ideas };
  }

  @Post('school-match')
  @ApiOperation({ summary: '学校匹配推荐' })
  async schoolMatch(@CurrentUser() user: CurrentUserPayload, @Body() data: ProfileAnalysisDto) {
    ...
    const messagesWithContext = [
      {
        ...
        content: '你是一位专业的留学申请顾问,专注于美国本科申请。请用中文回答问题,提供专业、友好的帮助。',
      },
      ...data.messages,
    ];
  ...
  async polishEssay(@CurrentUser() user: CurrentUserPayload, @Body() data: PolishEssayDto) {
    return this.aiService.polishEssay(data.content, data.style);
  }

  @Post('rewrite-paragraph')
  @ApiOperation({ summary: '段落改写' })
  async rewriteParagraph(@CurrentUser() user: CurrentUserPayload, @Body() data: RewriteParagraphDto) {
    return this.aiService.rewriteParagraph(data.paragraph, data.instruction);
  }

  @Post('continue-writing')
  @ApiOperation({ summary: '续写文书' })
  async continueWriting(@CurrentUser() user: CurrentUserPayload, @Body() data: ContinueWritingDto) {
    return this.aiService.continueWriting(data.content, data.prompt, data.direction);
  }

  @Post('generate-opening')
  @ApiOperation({ summary: '生成文书开头' })
  async generateOpening(@CurrentUser() user: CurrentUserPayload, @Body() data: GenerateOpeningDto) {
    return this.aiService.generateOpening(data.prompt, data.background);
  }

  @Post('generate-ending')
  @ApiOperation({ summary: '生成文书结尾' })
  async generateEnding(@CurrentUser() user: CurrentUserPayload, @Body() data: GenerateEndingDto) {
