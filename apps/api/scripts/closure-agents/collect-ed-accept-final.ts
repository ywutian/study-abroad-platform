...
const UNRESOLVED: Unresolved[] = [
  ...
  },
  {
    ...
    sourceUrl: 'https://www.gvsu.edu/admissions/undergraduate-application-23.htm',
  },
  {
...
async function main() {
  ...
  for (const r of RESOLVED) {
    if (!rangeOk(r.field, r.value)) {
      console.warn(`  SKIP ${r.name}: ${r.field} ${r.value} outside range gate`);
      continue;
    }
    ...
    await prisma.$executeRaw`
      ...
      WHERE id = ${r.targetId}`;
    targetUpdated += 1;
    console.log(`  CLOSED       ${r.name} :: ${r.field} => ${r.value}%  [${r.sourceUrl}]`);
  }
  ...
  for (const u of UNRESOLVED) {
    ...
    await prisma.$executeRaw`
      ...
      WHERE id = ${u.targetId}`;
    targetUpdated += 1;
    console.log(`  ${u.status.padEnd(12)} ${u.name} :: ${u.field} — ${u.reason}`);
  }
