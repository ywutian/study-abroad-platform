...
const BATCH: BatchTarget[] = [
  // ────────────────────────── CLOSED (6) ──────────────────────────
  {
    ...
    sourceUrl:
      'https://data.mst.edu/media/administrative/data/documents/cds/CDS%202024-2025%20v1.2.xlsx',
    confidence: 0.98,
    tier: 'OFFICIAL',
  },
  {
  ...
  },
  {
    ...
    sourceUrl:
      'https://www.csun.edu/sites/default/files/2026-04/CDS%202025%20v2.pdf',
    confidence: 0.95,
    tier: 'OFFICIAL',
  ...
  },
  {
    ...
    sourceUrl:
      'https://drive.google.com/uc?export=download&id=1L1IOSn19CxOwqRaDK8F_cZVzbsFD1EPg',
    lastError:
  ...
  },
  {
    ...
    sourceUrl:
      'https://oirds.rutgers.edu/CDS/2023/Newark%20CDS_2023-2024_final_V1.pdf',
    lastError:
  ...
  },
  {
    ...
    sourceUrl:
      'https://www.uvm.edu/d10-files/documents/2025-04/2024-2025-Common-Data-Set_0.pdf',
    lastError:
...
function assertBandsValid(t: ClosedTarget): void {
  ...
  for (const [band, frac] of Object.entries(b)) {
    if (frac < 0 || frac > 1) {
      throw new Error(
        `Sanity gate violation: ${t.name} band ${band}=${frac} (must be 0–1)`,
      );
    }
  }
}
  ...
  for (const t of BATCH) {
    ...
    if (isClosed(t)) {
      ...
      if (!school) {
        ...
        schoolsUpdated += 1;
        console.log(
          `  OK   ${t.name} => ${JSON.stringify(t.bands)}  [${t.sourceUrl}]`,
        );
      }
    } else {
