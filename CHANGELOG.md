...
- **时间线归档按申请季隔离** — 学校申请新增 Fall 入学年份，旧周期不再阻止新周期；历史截止日不再自动滚到下一年，个人事件按截止日与活动日中较晚者归档，Web/Mobile 归档统一只读
- **全平台 API E2E 建库流程** — 专用测试库先安全重置，再执行 `prisma migrate deploy` 与 `db push`；迁移独有的数据库约束现在会进入 E2E 验证
