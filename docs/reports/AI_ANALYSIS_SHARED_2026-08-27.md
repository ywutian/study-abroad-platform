...
## 重新认证后的续查（2026-08-27 15:48–15:54 UTC）

Google认证已恢复，描述与日志查询均成功，无需新增IAM；上文认证阻断属于历史状态。失败版本01003-por有7条直接上游OpenAI HTTP 401及7条传播错误，当前100%版本00992-zin近24小时也有5条直接上游403。两版本引用同一个凭据，Secret最新版本创建于2026-06-04，但未读取其内容，不能据此确认密钥归属或是否已被上游撤销。HTTP健康不能代替模型调用验收。

原只读脚本仅查ERROR级别，实际Nest错误被Cloud Logging记为DEFAULT，造成漏报。现已本地修复为服务范围内的ERROR或明确失败消息匹配，保留只输出分类/计数的边界；回归先红后绿，相关15项测试通过。查询分组依据[Google Logging查询语法](https://docs.cloud.google.com/logging/docs/view/logging-query-language)。未复制原始云日志或密钥值。

0ae25487的最终CI 33066959105已成功，API 4,442项测试通过；此后日志修复的远端CI尚未执行。新的待确认事项是生产聊天接口与专用Secret方案，不再是Google登录。建议独立配置经授权的Relay聊天凭据，避免直接改全局配置影响Embedding；尚未获得Secret变更授权，未实施该方案。候选实测失败仍保留，不能因认证恢复而默认启用或跳过发布验收。
