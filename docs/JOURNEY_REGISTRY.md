...
| 字段                  | 值                                          |
| --------------------- | ------------------------------------------- |
| `registry_version`    | `2026-04-10.v4`                             |
| `owner`               | Release / QA                                |
...
| `SJ-5`     | `temporary-child` | Teams 组队卡：建卡 → 发布 → Swipe → Match → 邀请/入队 | applicant      | web     | `codex`                 | `objective`     | no             | yes                | 布局、专业感       | 组队匹配主子旅程；验证邀请送达、join link fallback 与入队接受闭环                                      |
