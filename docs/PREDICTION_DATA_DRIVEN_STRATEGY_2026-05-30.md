...
**intlAcceptanceRate data-quality incident (found + fixed).** ⚠️ **SUPERSEDED — the `intl >= overall` ⇒ contamination conclusion in this paragraph was disproven; see the CORRECTION note immediately below.** Auditing the _existing_ 77% coverage (`scripts/audit-intl-rate-quality.ts`) found **56 contaminated values**: `intl >= overall` (statistically impossible at selective schools) or a fraction-vs-percent scale mismatch. Root cause: international ENROLLMENT % (`intlStudentPct`, separately 100%-covered) and overall-rate relabels leaked into the admit-rate column via an early metadata migration. Impact: the counselor `intlMultiplier` (`ratio = intl/overall`, clamped [0.3, 1.2]) turned a correct ~×0.5 international _penalty_ into a wrong **×1.2 boost** at elite schools (Yale 11%/4.8%, Caltech 9%/2.6%, Stanford 8%/3.8%, Dartmouth, Northwestern …) — telling international applicants their odds were _higher_ than domestic, on exactly their dream schools. Fix: `prisma/seed-intl-rate-correction.ts` nulls contaminated values (→ engine falls back to the correct selectivity heuristic), wired into the seed pipeline after the intl writers; `scripts/audit-intl-rate-quality.ts` is the regression gate (exit 1 on any `intl >= overall`). Local DB: **194 → 138 clean intl rates, gate PASS**. Apply to prod by running the correction standalone. This is the canonical example of why "data-grounded" requires _auditing_ the data, not just having it.

> **CORRECTION (2026-06, #351/#355).** The `intl >= overall` ⇒ contamination premise above was **DISPROVEN**. Web research verified 6/6 such schools (UC Davis 50.7%, Ohio State 72%, U Minnesota 82%, Rutgers 70.7%, Virginia Tech 66%, U Florida 32.5%) against **published international admit rates** — all real, **0/6** matched the enrollment %. `intl >= overall` is the genuine **revenue-seeking-public** pattern, so the old directionality nulling was destroying real signal on exactly the schools international applicants care about. The correction + regression gate are now **SCALE-ONLY** (null only `intl <1%` / `>100%`); `intl >= overall` is kept. The directionality script `audit-intl-rate-quality.ts` was **removed**, superseded by `scripts/audit-prediction-data-integrity.ts` (wired into the CI prediction-gate + prod `migrate.sh`). The deeper lesson stands — audit the data — but the _invariant itself_ must also be audited: a plausible-sounding rule ("intl is always more competitive") was wrong for a whole school class.
