...
| FR-008  | 镜像OpenSSL最低3.5.8-r0；发布依赖Docker扫描与SBOM通过，实际部署摘要必须在迁移前通过同等扫描 | [DECISION]  |
...
| AC-007 | FR-005,FR-008  | main推送和授权手动发布均受扫描/SBOM约束；扫描失败不得迁移/部署；只读inspection不发布；负面测试拒绝移除依赖、放宽扫描或改变摘要/顺序 |
...
[RUNTIME] 2026-08-27安全续轮：PR #633已合并为0982c5de；main CI 33095225453发现CVE-2026-14456（libcrypto3/libssl3/openssl 3.5.7-r0，Trivy三个HIGH）后主动取消。生产00992-zin仍100%，未创建新Revision。已启动的迁移study-abroad-migrate-pc2b2在17:11:40Z独立成功结束；取消CI不等于取消已启动云任务，不恢复或盲目重跑数据库。

[DECISION] FR-008/AC-007由Codex在当前上线授权内修复：Dockerfile两阶段固定安全版本下限，ci.yml统一发布资格并增加docker/sbom依赖，在实际Artifact Registry摘要上执行现有Trivy硬门禁，顺序先于迁移；不新增忽略项，不改IAM/凭据/业务预算。补充门禁负例和配置漂移校验，再全量CI与新版本生产闭环。此项实施前intake必须通过。

[EXTERNAL] 修复依据：[OpenSSL 3.5发布说明](https://mirror.openssl-library.org/news/openssl-3.5-notes/)及[Alpine v3.23 x86_64包](https://pkgs.alpinelinux.org/package/v3.23/main/x86_64/openssl)确认3.5.8/3.5.8-r0已发布。上游与扫描器严重度评级不同，不据此降低现有HIGH/CRITICAL门禁。
