...
## 20260811120000_application_timeline_cycle_history

申请时间线历史周期修复：

- `ApplicationTimeline` 新增非空 `applicationYear`，表示 Fall 入学年份
- 唯一键由 `userId + schoolId + round` 扩展为 `userId + schoolId + round + applicationYear`
- 历史行按截止日回填；8–12 月截止日期归入下一 Fall 入学年，无截止日记录按创建时间所属申请季回填
- 数据库默认值按当前日期推导申请季，保证旧、新服务版本重叠期间的写入兼容
- Prisma 事务内使用常规索引创建，并设置 5 秒 `lock_timeout` 与 5 分钟 `statement_timeout`；锁繁忙时迁移安全失败并可重试
- 应用层不再把历史学校截止日滚到下一年；归档记录由 API 强制只读

本地开发库如果曾使用 `prisma db push` 且缺少 `_prisma_migrations`，必须先用 `prisma migrate diff` 确认零差异，再按 [ADR-0002](../adr/0002-prisma-migration-manual-resolve-strategy.md) 执行 baseline；禁止直接重放全部历史迁移。
