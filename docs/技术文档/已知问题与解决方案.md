...
# 验证 Turbopack 模式（默认）
pnpm --filter web dev
# 访问 http://localhost:4100/zh/dashboard 应返回 200
...
**严重程度**: P2 Medium
**状态**: ✅ 已解决（当前实现复核 2026-08-12）

历史版本中 API 返回学校数据而前端列表为空。当前前端通过
`buildSchoolQueryParams()` 发送 `page` / `pageSize`，后端 `PaginationDto` 使用同名字段；
`apiClient` 统一解包 `{ success, data }` 响应。

### 当前验证

- `school-filters.test.ts` 覆盖分页与筛选参数序列化。
- `/schools`、`/schools/[id]`、`/schools/compare` 均列入全表面路由门禁，其中学校列表为
  critical surface。
- Web 测试、类型检查与生产构建通过。

若未来再次出现空列表，按 API 响应、查询参数和数据库内容重新诊断，不再沿用历史
`limit`/`take` 假设。
...
- [PREDICTION_SYSTEM.md](../PREDICTION_SYSTEM.md)
- [ADR-0008](../adr/0008-prediction-multi-engine-ensemble.md)
