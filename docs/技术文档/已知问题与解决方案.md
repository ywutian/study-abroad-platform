...
Next.js 16.1.3 的 Turbopack 编译器在处理路由组 (`(main)`, `(auth)`) 时存在兼容性问题。所有位于路由组内的页面在 Turbopack 模式下返回 404，但在 Webpack 模式下正常工作。
...
**说明**：默认使用 Webpack 编译器确保稳定性，`dev:turbo` 提供 Turbopack 选项供测试。
