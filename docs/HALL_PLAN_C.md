...
| 序  | 项                                                                                                      | effort | 状态                | 说明                                                                                                                                                                                                                                        |
| --- | ------------------------------------------------------------------------------------------------------- | ------ | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **C** 立即全量 `acceptPeerReview=false` 止血                                                            | S      | ✅ 已完成 (PR #239) | 现行 PIPL 违规，纯 UPDATE。迁移 `20260519090536`：default `true→false` + 全量 `UPDATE` + 清 `hallPublicProfile` 快照                                                                                                                        |
| 2   | **B+C6** `review` 整体退役 + 删全部已弃用列（`Review`/评分列/`acceptPeerReview`/`SwipeStats` 游戏化列） | M      | ✅ 已完成 (PR #240) | 一次破坏性迁移 `20260519100000`（DROP `Review`/`ReviewReaction` 表 + `acceptPeerReview` + `SwipeStats` 游戏化列）。`reviewerLevel/Credit/QualifiedAt`、`hallAvgRating/hallReviewCount` 作为孤儿列保留（不在 C6 清单），留待后续非 Hall 清理 |
| 3   | **A** 无操作                                                                                            | —      | ✅ 无需操作         | 保留 `ranking` C1 现状，本项不再是任务                                                                                                                                                                                                      |
| 4   | **D-试点** ~30–50 案例录入 `path`，埋点测浏览参与度                                                     | S      | ⏳ 待办（非代码）   | 无新代码，2–3 周                                                                                                                                                                                                                            |
| 5   | **闸门** 试点信号 → 过则建 XL 管线 + **E**（含定档功效分析）                                            | XL     | ⏳ 待 D 试点        | 不过则 `verified` 不单独成 tab                                                                                                                                                                                                              |
| 6   | **F** 据试点结果决定是否解散 Hall 壳                                                                    | —      | ⏳ 待用户拍板       | 需用户拍板；命名错位文案可单独修                                                                                                                                                                                                            |

> **执行记录（2026-05-19）**：C、B+C6 已由多 agent 实施 + 验收完成。`review` 功能（锐评 tab、
> 全部 `/halls/reviews*` + `/reviewer/*` 端点、L1→L2 资质测验、AI 锐评教练、`/admin/reviews`、
> `settings/peer-review` 开关页、移动端锐评屏）整体退役；Hall IA 收缩为 3 tab：`verified` /
> `ranking` / `path`。剩余仅 D/闸门/F 三项非代码决策项。
