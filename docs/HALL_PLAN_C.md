...
| 批次   | 目标                                         | 折叠的安全项                                                                            | 量级 | 状态                           |
| ------ | -------------------------------------------- | --------------------------------------------------------------------------------------- | ---- | ------------------------------ |
| **C1** | `ranking` 去评分                             | —                                                                                       | M    | ✅ 已完成（commit `625f71ce`） |
| **C2** | `review` 砍数字打分 → 纯定性                 | B1 同意闸门 + B2 opt-out 过滤 + <16 硬下限 + 写接口限流 + HALL_REVIEWER_SELECT 删 email | M    | 待做                           |
| **C3** | `path` 去游戏化                              | —                                                                                       | M    | 待做                           |
| **C4** | `verified` 修复                              | —                                                                                       | M    | 待做                           |
| **C5** | 安全：脱敏快照接入 + verified-ranking 匿名化 | B3 + B4 + swipe PII + getPublicLists email                                              | L    | 待做                           |
| **C6** | 迁移：删已弃用的列                           | —                                                                                       | S    | 待做（延后，C2/C3 稳定后）     |
