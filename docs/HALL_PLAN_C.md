...
## 七、§7 五项决策 —— 已拍板（5-agent 辩论 + 综合）

2026-05-19，由 study-abroad / applicant-simulator / architect / security-reviewer /
data-model-reviewer 5 个 agent 各自独立分析全部 5 项后综合裁决。

### A. Tab 数量 → **合并 `ranking` 进 `verified`，Hall 收成 3 tab（与 D 捆绑做）**

4/5 票合并、1 票中立偏合并。独立 tab 本身即「权威」暗示，家长分不清「录取数据中心」
与「同伴定位」。时机：现在 `verified` 仍空，并入空 tab = 半空 tab 且搅动刚验收的代码
——故**终态定为 3-tab，但在 D 重做 `verified` 界面时一并落地**。

### B. `review` tab → **降级，不删除**

分歧最大（2 砍 / 3 留）。综合：把 `review` 从顶级 tab 撤下，定性反馈能力保留为
「从档案页发起」的可选动作；**不删 `Review` 模型、不做 DROP TABLE**（C2b 已移除有害的
数字打分，剩余功能零维护成本；删它要付 L 级破坏性迁移）。A+B 落地后 **Hall = 2 tab
（verified + path）**，正好回到 §二「数据中心 + 案例学习」终态。

### C. `acceptPeerReview` → **改 opt-in（默认 `false`），迁移把【所有】存量用户置 false** 🔴

5/5 一致改 opt-in。迁移范围以 security 论证为准（PIPL 合规问题，security 有权威）：
① `profile.birthday` 可空自报，「仅未成年人」迁移无法可靠识别全部未成年人；② opt-out
默认下无任何用户给过 PIPL「自愿、明示」同意 → 全量重置才合法干净。配套：同事务清空
`hallPublicProfile` 快照、记录带时间戳+版本的同意事件、<14 岁走监护人同意。
**这是 5 项里唯一「已并入生产代码的现行违规」——优先级 #1（修复）。effort S。**

### D. 真实大陆录取数据管线 → **投入建设，这是护城河** 🎯

强共识 YES——唯一「创造价值」的项。架构：**复用现有 `case` + `verification` 模块，
不新建采集、不爬虫**。① 管理员/编辑部 L1 录入做冷启动（只进 `path` 案例库）；② 用户提交
录取信走已有 `VerificationRequest` 流程 → 专家核验升 L2/L3（竞品爬不到的真护城河）。
安全 day-1 硬门槛：发布同意（独立于注册）、未成年人监护人同意、入库即脱敏、k-匿名、
可撤回删除、来源审计。数据门槛：去重、异常值检测、核验 SLA、`verifiedBy` 溯源。
effort **XL**——战略 #1 但最慢，不阻塞 C/E。

### E. `verified` 可靠度阈值 → **收紧，样本不足直接隐藏（不是「打折标」）**

5/5 收紧。阈值身兼二职：统计有效性 + k-匿名（小学校单条记录可反推真人，尤其未成年人）
——故须门控「显示」本身。采用 `A≥15 / B≥8 / C≥3 / n<3 不渲染该卡`；C 档（3–7）只显示
原始计数、不显示任何百分率。effort S，视觉收益待 D 有真实数据才显现。

### 推荐执行顺序

| 序  | 项                            | effort | 理由                                                                                       |
| --- | ----------------------------- | ------ | ------------------------------------------------------------------------------------------ |
| 1   | **C** opt-in + 全量迁移       | S      | 现行 PIPL 违规，必须先修                                                                   |
| 2   | **C6** 删已弃用列             | S      | `Review` 评分列仍 `NOT NULL` 而 DTO 已改可选，schema/DTO 矛盾有潜在 500 风险，不能无限期拖 |
| 3   | **E** 收紧可靠度阈值          | S      | 正确且自包含                                                                               |
| 4   | **B** review 降级（非破坏性） | M      | —                                                                                          |
| 5   | **D** 真实数据管线            | XL     | 护城河；启动架构 + 同意设计                                                                |
| 6   | **A** 合并成 3-tab            | M      | 随 D 重做 `verified` 时一起做                                                              |
